var app = new Vue({
  ...
  data: {
    ...
    //task_management
    user_tasks: [
      {code: 'download', name: 'Download today’s files for verification from mail'},
      {code: 'inspect', name: 'Verify downloaded files with EDF Inspector'},
      {code: 'send', name: 'Send verified files via mail'},
      {code: 'sleep', name: 'Turn to sleep mode'}],
    //desktop
    desktop_icons: [
      {image: "./image/desktop/desktop_bin.svg", name: "Bin"},
      {image: "./image/filetypes/file_foler.svg", name: "My Folder"}
    ],
    //notification center
    notifications: {
      'mail': {
    ...
    app_mail: {
      mail_contacts: [
        ...
        {name: "Tsun Li-zhou", date: "2019/10/28 8:57 PM", account: "lairotut@ptnp.com"}, //1
        {name: "Akatoshi Ootsunomiya", date: "2019/06/28 9:27 AM", account: "ofa_afo@ptnp.com"}, //2
        {name: "Nata Rion", date: "2019/10/25 3:22 PM", account: "gabby_natar@ptnp.com"}, //3
        {name: "Young Deok, Lim", date: "2019/01/14 3:33 PM", account: "dragonvirtue@ptnp.com"}, //4
        {name: "Rudy Sanchez", date: "2019/09/04 1:38 PM", account: "ays@ptnp.com"}, //5
        {name: "Thierry Martiez", date: "2019/10/28 5:57 PM", account: "thierry_securite@ptnp.com"}, //6
        {name: "Fred Martin", date: "2019/05/31 5:14 PM", account: "anniversaire@ptnp.com"} //7
      ],
      my_account: 'tom0824@ptnp.com',
      mail_data: {
        "GM": [],
        "Tsun Li-zhou": [mail_pre_1],
        "Nata Rion": [mail_pre_3],
        "Akatoshi Ootsunomiya": [mail_pre_4],
        "Thierry Martiez": [mail_pre_2],
        "Young Deok, Lim": [mail_pre_5],
        "Rudy Sanchez": [mail_pre_6],
        "Fred Martin": [mail_pre_7_3, mail_pre_7_4, mail_pre_7_5],
      },
      ...
      tabs: [
        {
          ...
          history: [
            {title: 'Intranet', address_deep: './web/intranet_en.html'},
          ]
        },
        {
          current_pivot: 0,
          ...
          is_history_move: false,
          history: [
            {title: 'Search', address_deep: './web/search_en.html'},
          ]
        },
      ],
      wrong_permission: false
  ...
  methods: {
    ...
    mail_download_attachment: function (d) {
      ...
      if (d.constructor.name == "File" && !( this.has_plugin('vaccine.plg') && d.extension == 'plg' )) {
        ...
        this.fire_notification('download', {title: d.get_fullname()+' Download complete.', desc: ThisTime.get_now_format('s')})
        this.go_download();
        miro_sound_play("okay");
      } else if ( this.has_plugin('vaccine.plg') && d.extension == 'plg' ) {
        this.fire_notification('download', { 
          title: 'Download blocked by vaccine.',
        ...
        setTimeout(function() {
          if (!e.file.password_locked) {
            ...
            if (was_decryption){
              ...
              self.fire_notification('inspector', {title: 'File decoding completed.', desc: ThisTime.get_now_format('s')});
            } else if (e.result != 'locked') {
              if (i == file_length) {
                miro_sound_play("okay");
              }
              self.fire_notification('inspector', {title: 'File verifcation completed.', desc: ThisTime.get_now_format('s')});  
            }
          } else {
            e.result = "locked";
          }
        }, 75*i + ( e.file.get_size() > 5000 ? 5000 : e.file.get_size() ) );
      });
    },
    ...
    inspector_enable_decryption: function() {
      ...
      if (dev == "Jeremy Foucault" && pro == "Social Network Privacy" && ver=="v2.03b") {
        miro_sound_play("okay");
        let devdata = this.app_inspector.development_data;
        for (let i = 0; i < devdata.length; i++) {
          if (devdata[i].is_encrypted && inspection_decrypt(devdata[i].name) == "Jeremy Foucault") {
        ...
        if (!ending4.is)  {
          this.app_inspector.setting_wrong = [];
          if (dev != "Jeremy Foucault") this.app_inspector.setting_wrong.push("dev");
